...
namespace Armature.Core
{
  ...
  /// </summary>
  public abstract record IsInjectPointByAttributePattern<T> : IUnitIdPattern where T : Attribute
  {
    private readonly Predicate<T>? _predicate;

    /// <param name="predicate">Predicate matches the attribute for a desired state</param>
    [DebuggerStepThrough]
    protected IsInjectPointByAttributePattern(Predicate<T>? predicate) => _predicate = predicate;
