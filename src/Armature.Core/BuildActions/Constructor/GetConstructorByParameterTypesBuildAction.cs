...
namespace Armature.Core.BuildActions.Constructor
{
  public class GetConstructorByParameterTypesBuildAction : IBuildAction
  {
    ...
    [DebuggerStepThrough]
    public override string ToString() => string.Format(LogConst.OneParameterFormat, GetType().GetShortName(), string.Join(", ", _parameterTypes.Select(Log.AsLogString).ToArray()));
  }
}