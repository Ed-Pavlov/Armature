  ...
  public class OrderedBuildActionContainer : IBuildAction, IEnumerable
  {
    ...
    public void Process(IBuildSession buildSession)
    {
      foreach (var buildAction in _buildActions)
      {
        try
        {
          ...
          {
            Log.Info("redirected execution to {0}", buildAction);
        ...
        {
          Log.Trace("User exception was throw during executing {0}", buildAction);
          LogException(exc);
          throw;
        }
      }
    ...
    private static void LogException(Exception exc)
    {
      using (Log.Block(LogLevel.Trace, "Exception: {0}", exc))
        foreach (DictionaryEntry entry in exc.Data)
          Log.Trace("{0}: {1}", entry.Key, entry.Value);
    }
    ...
    public void PostProcess(IBuildSession buildSession)
    {
      if (_effectiveBuildAction != null)
      {
        ...
        Log.Info("redirected execution to {0}", _effectiveBuildAction);
